...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////Users/saiharshinigarapati/Desktop/login-register-form/database.db'
db = SQLAlchemy(app)
...
def connect():
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS user_groups (id VARCHAR(255) PRIMARY KEY,group_id VARCHAR(255), user_id VARCHAR(255),file_path VARCHAR(255),name VARCHAR(255),description VARCHAR(255),current_dt VARCHAR(255))")
...
def download_group_file(id):
    conn = sqlite3.connect('user.db')
    cur = conn.cursor()
    query = f"select * from user_groups where id='{id}'"
    cur.execute(query)
    rows = cur.fetchone()
    print(rows)
    conn.commit()
...
def view_group_files(group_id):
    ...
    conn = sqlite3.connect('user.db')
    cur = conn.cursor()
    query = f"select * from user_groups where group_id='{group_id}'"
    cur.execute(query)
    rows = cur.fetchall()
    print(rows)
    conn.commit()
...
def dashboard():
    ...
    # query1 = f"select us.name,us.username from user as us left join friend as fr on user.id=fr.friend_id where fr.user_id = '{userID}' "
    # query2 = f"select * from friend where user_id='{userID}' or friend_id='{userID}'"
    ...
    #     for fr in friends:
    #         # friends_lst.append(fr[2])
    #         print(fr)
    #         query3 = f"select * from user where id != '{userID}' and id != '{fr[2]}' "
    #         cur.execute(query3)
    #         print('======',[cur.fetchone()])
    #         friends_lst.append(cur.fetchone())
    #     rows = friends_lst
    # print(friends_lst)
    # print(rows)
...
#         conn = sqlite3.connect('user.db')
#         cur = conn.cursor()
#         query = f"update friend_files set  where id='{file_id}'"
#         cur.execute(query)
#         rows = cur.fetchone()
#         print(rows)
